...
public interface LessonMapper extends MyMapper<Lesson> {
    ...
    @Select("SELECT id,name,firstPath,secondPath,thirdPath FROM lesson")
    Page<BriefLesson> getBriefLesson();

    @Select("SELECT id,firstPath,secondPath,thirdPath,name FROM lesson WHERE id=#{id}")
    BriefLesson getLessonById(@Param("id")Long id);
    //提供给其他"课程收藏"、"已购课程"使用的接口

    @Select("SELECT id,name,firstPath,secondPath,ThirdPath FROM lesson WHERE content LIKE CONCAT('%',#{firstPath,jdbcType=VARCHAR},'%') OR CONCAT('%',#{secondPath,jdbcType=VARCHAR},'%') OR CONCAT('%',#{thirdPath,jdbcType=VARCHAR},'%') OR CONCAT('%',#{name,jdbcType=VARCHAR},'%')")
    ...
    @Select("SELECT id,name,firstPath,secondPath,ThirdPath FROM lesson WHERE name=#{name}")
